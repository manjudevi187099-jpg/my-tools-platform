...
export default function ProfessionalPdfEditor() {
  ...
  const loadNewFile = async (newFile: File, pwd = '') => {
    ...
    try {
      ...
      // Reset only if it's a completely new root file upload, keep annotations if merging (handled inside merge)
      setAnnotations([]); setDeletedPages([]); setPageRotations({});
      setGlobalWatermark(''); setExportRange(''); setMetaTitle(''); setMetaAuthor('');
  ...
  const renderPage = async (pdf: pdfjsLib.PDFDocumentProxy, pageNum: number, rotation: number = 0) => {
    try {
      ...
      const targetBaseWidth = isLandscape ? 850 : 700; 
  ...
  const getMouseCoords = (e: React.MouseEvent<HTMLDivElement>) => {
    const canvas = canvasRef.current;
    if (!canvas || !pdfDimensions) return { x: 0, y: 0 };
    const rect = canvas.getBoundingClientRect();
    return {
      x: ((e.clientX - rect.left) / rect.width) * pdfDimensions.w,
      y: ((e.clientY - rect.top) / rect.height) * pdfDimensions.h
    };
  };
